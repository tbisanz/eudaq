...
class USBPixI4ConverterBase : public ATLASFEI4Interpreter<dh_lv1id_msk, dh_bcid_msk>{
  ...
  protected:
	...
	std::vector<std::vector<int> > moduleConfig;
	std::vector<std::vector<int> > moduleCount;
	std::vector<std::vector<int> > moduleIndex;
	bool advancedConfig;
	...
	unsigned int dataBlocksPerEvent = 0;
	mutable unsigned int dataBlocksProcessed = 0;

	unsigned int instanceNo() const { 
		return dataBlocksProcessed%dataBlocksPerEvent;
	}
	...
	void getBOREparameters(const Event & ev) {
		...
		std::cout << "consecutive_lvl1: " << consecutive_lvl1 << std::endl;	
		...
		std::cout << "first_sensor_id: " << first_sensor_id << std::endl;	

		tot_mode = ev.GetTag("tot_mode", 0);
		std::cout << "tot_mode: " << tot_mode << std::endl;	
		...
		for (unsigned int i=0; i<count_boards; i++) {
			...
			std::cout << "boardid_" + to_string(i) +": "  << ev.GetTag ("boardid_" + to_string(i), -1) << std::endl;	
		}

		const char* module_setup = ev.GetTag("modules", "").c_str();
		std::cout << "modules: "  << ev.GetTag("modules", "") << std::endl;	
		char* module_setup_copy = strdup(module_setup);
		char* subptr = nullptr;
		subptr = strtok(module_setup_copy, " ,");

		while(subptr != nullptr) {
			moduleConfig.at(this->dataBlocksPerEvent).push_back( atoi(subptr) );
			if(moduleConfig.size() > 1 ) {
				int offset = moduleConfig.at(this->dataBlocksPerEvent-1).back();
				for(int& i: moduleConfig.at(this->dataBlocksPerEvent)) {
					i+= offset;
				}
			}
		...
		if( moduleConfig.at(this->dataBlocksPerEvent).size() > 0) {
			
			int previousModule = moduleConfig.at(this->dataBlocksPerEvent).at(0);
			int counter = 0;
			...
			for(int i: moduleConfig.at(this->dataBlocksPerEvent)) {
				if( i == previousModule) {
					...
					moduleCount.at(this->dataBlocksPerEvent).push_back(counter);
					counter=1;
				}
				previousModule = i;
			}
			...
			std::cout << "moduleCount:" << std::endl;
			for(int i: moduleCount.at(this->dataBlocksPerEvent)) std::cout << i << std::endl;
			std::cout << "moduleConfig:" << std::endl;	
			for(int i: moduleConfig.at(this->dataBlocksPerEvent)) std::cout << i << std::endl;
				
			//sanity check
			if(moduleCount.at(this->dataBlocksPerEvent).size() != moduleConfig.at(this->dataBlocksPerEvent).back()) {
				std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
			...
			for(int value: moduleCount.at(this->dataBlocksPerEvent)) {
				for(int j = 1; j <= value; j++) {
					...
					moduleIndex.at(this->dataBlocksPerEvent).push_back(j);
				}
			}
		} else {
				std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
	...
	StandardPlane ConvertPlane(const std::vector<unsigned char> & data, unsigned id) const {
		...
		if(advancedConfig) {
			int chipCount = moduleCount.at(this->instanceNo()).at(id-1);
			if(chipCount > 2) {
				colMult = 2;
				rowMult = 2;
			} else if(chipCount == 2) {
		...
		for(size_t i=0; i < data.size()-8; i += 4) {
			...
			if(this->is_dh(Word)) {
				...
				if(getHitData(Word, false, Col, Row, ToT)) {
					if(advancedConfig) transformChipsToModule(Col, Row, moduleIndex.at(this->instanceNo()).at(id-1) );
...
class USBPixI4ConverterPlugin : public DataConverterPlugin , public USBPixI4ConverterBase<dh_lv1id_msk, dh_bcid_msk> {
  ...
  public:
	...
	virtual void Initialize(const Event& bore, const Configuration& /*cnf*/) {
		//This ensures that we start counting at data block 0
		this->moduleConfig.push_back( std::vector<int>() );
		this->moduleCount.push_back( std::vector<int>() );
		this->moduleIndex.push_back( std::vector<int>() );
		
		this->getBOREparameters(bore);
		++(this->dataBlocksPerEvent);
		std::cout << "Initialize called - " << this->dataBlocksPerEvent << std::endl;
		this->dataBlocksProcessed = (this->dataBlocksPerEvent)-1;
	}
	...
	bool GetStandardSubEvent(StandardEvent & sev, const Event & ev) const {
		...
		for(size_t i = 0; i < ev_raw.NumBlocks(); ++i) {
			if(this->advancedConfig) {
				sev.AddPlane(this->ConvertPlane(ev_raw.GetBlock(i), this->moduleConfig.at(this->instanceNo()).at(i)));
			} else {
	...
	virtual bool GetLCIOSubEvent(lcio::LCEvent & lcioEvent, const Event & eudaqEvent) const {		
		...
		if(this->advancedConfig) previousSensorID = this->moduleConfig.at(this->instanceNo()).at(0)+chip_id_offset-1;
		...
		for(size_t chip = 0; chip < ev_raw.NumBlocks(); ++chip) {
			...
			if(this->advancedConfig) {
				sensorID = this->moduleConfig.at(this->instanceNo()).at(chip)+chip_id_offset-1;
			} else {
			...
			for(size_t i=0; i < buffer.size()-4; i += 4) {
				...
				if( this->is_dh(Word)) {
					...
					if(this->getHitData(Word, false, Col, Row, ToT)) {
						if(this->advancedConfig) this->transformChipsToModule(Col, Row, this->moduleIndex.at(this->instanceNo()).at(chip));
